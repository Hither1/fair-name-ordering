...
class Node:
    ...
    blocks: List[Optional[Block]]  # keep a track of the blockchain
    ...
    # the matrix to store queues, one queue for each node
    enc_wit_queues = List[List[Optional[Point]]]
    ...
    _t_vote_cert_phase_start: float
    ...
    def run(self):
        """ Main entry point.
            Waits until the protocol should start and then runs the GRandPiper protocol.
    ...
    def propose(self):
        ...
        self.datasets[self.round] = block
        self.shared_secret = new_shared_secret
        self.confirmation_counter.update([block.header_digest])
    ...
    def vote(self):
        ...
        msg = VoteMessage(self.ID, self.dataset, self.propose_message.dataset_header_signature)
        self.broadcast_message(msg)
    ...
    def process_vote(self, msg: VoteMessage):
        assert (
            self.dataset
    ...
    def create_certificate(self, dataset_header_digest: Hash):
        ...
        self.certificates[self.round] = cc
    ...
    def verify_proposal(self, msg: ProposeMessage):
        ...
        if not ed25519.verify_detached(
                block.serialized_header, msg.dataset_header_signature.serialized, NODE_INFOS[msg.sender].public_key
        ):
            return False
